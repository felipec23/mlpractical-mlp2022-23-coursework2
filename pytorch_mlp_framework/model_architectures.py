...
class ConvolutionalProcessingBlockBatchNorm(nn.Module):
    ...
    def build_module(self):
        ...
        print("Layer dict:", self.layer_dict)
        self.layer_dict['conv_0'] = nn.Conv2d(in_channels=out.shape[1], out_channels=self.num_filters, bias=self.bias,
                                              kernel_size=self.kernel_size, dilation=self.dilation,
                                              padding=self.padding, stride=1)
        ...
        # Applying batch normalization in layer 0, before relu
        self.layer_dict['bn_0'] = nn.BatchNorm2d(num_features=out.shape[1])
        print("Layer dict 2:", self.layer_dict)
        ...
        # Applying batch normalization in layer 1, before relu
        self.layer_dict['bn_1'] = nn.BatchNorm2d(num_features=out.shape[1])
        print("Layer dict 3:", self.layer_dict)
...
class ConvolutionalProcessingBlockBatchNorm_rc(nn.Module):
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation, stride=1, downsample=None):
        super(ConvolutionalProcessingBlockBatchNorm_rc, self).__init__()
...
class ConvolutionalDimensionalityReductionBlockBatchNorm_rc(nn.Module):
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation, reduction_factor,stride=1, downsample=None):
        super(ConvolutionalDimensionalityReductionBlockBatchNorm_rc, self).__init__()
